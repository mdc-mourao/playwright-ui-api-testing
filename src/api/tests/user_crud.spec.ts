...
import { userCreateSchema, userListSchema, userUpdateSchema, validateUserData } from '../schemas/userSchemas';
...
test.describe('@A1 @API - CRUD Operations', () => {
    ...
    test('GET: Verify a single users data structure and validate specific fields', async() => {
        ...
        const targetId = listInfo.data[Math.floor(Math.random() * listInfo.data.length)].id;

        const { response: userResponse, info: userInfo } = await userClient.getUser(targetId);
    ...
    for (const user of userDatasets) {
        test(`Data-Driven POST: Create user ${user.name} with role: ${user.job}`, async ({}) => {
            ...
            const { response, info } =  await userClient.createUser(user.name, user.job);
    ...
    test('PUT: Update an existing user and validate the response reflects the changes', async() => {
        ...
        const { response: listResponse } = await userClient.getUsersList(`page=${pageNumber}`);
        ...
        const { response: updateResponse, info: updateInfo } = await userClient.updateUser(targetId, name, job);
    ...
    test('DELETE: Verify the response returns a 204 status code', async() => {     
        const page = apiFactory.getRandomPage();   
        const { response: listResponse } = await userClient.getUsersList(`page=${page}`);
        const listBody = await listResponse.json();
        const targetId = listBody.data[Math.floor(Math.random() * listBody.data.length)].id;
